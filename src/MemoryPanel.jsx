...
export default function MemoryPanel({messages=[],memories,candidates,onCandidates,onMemories,onSaved=()=>{},disabled}){
 ...
 <label>{names[c.type]||'此前留下的内容'}<textarea disabled={disabled} value={c.text} onChange={e=>onCandidates(candidates.map(x=>x.id===c.id?{...x,text:e.target.value}:x))}/></label>
 ...
 <button className="text-button" disabled={disabled||candidates.some(c=>!c.text.trim()||!c.type)} onClick={()=>{let next=[...memories];for(const c of candidates){if(!candidateHasUserSource({messages},c))continue;next=next.filter(m=>m.id!==c.id&&m.id!==c.replacesId);if(!next.some(m=>sameMemory(m.text,c.text)))next.push({...c,savedAt:new Date().toISOString()});}if(next.length>30){setError('记忆已满，请先删去不再需要的内容。');return;}setError('');onMemories(next);onCandidates([]);onSaved();}}>保存这些内容</button></section>}
 {['user','fiction'].map(group=><section key={group}><h3>{group==='user'?'关于你的记忆':'Ta的经历与计划'}</h3>{memories.filter(m=>group==='user'?m.type!=='fiction':m.type==='fiction').map(m=><article className="memory-item" key={m.id}>
 <span className="tag">{names[m.type]}</span>
 ...
 {editing===m.id?<><button className="text-button" disabled={disabled||!text.trim()} onClick={()=>{onMemories(memories.map(x=>x.id===m.id?{...x,text:text.trim()}:x));setEditing(null);onSaved();}}>保存</button><button className="text-button" onClick={()=>setEditing(null)}>取消</button></>:<button className="text-button" disabled={disabled} onClick={()=>{setEditing(m.id);setText(m.text);}}>编辑</button>}
