...
export default function MemoryPanel({messages=[],memories,candidates,onCandidates,onMemories,onSaved=()=>{},onConfirm,disabled}){
 ...
 {c.requiresConfirmation&&<label>请选择要替换的记忆，并把下方文字改成准确的新内容<select value={c.confirmTargetId||''} onChange={e=>onCandidates(candidates.map(x=>x.id===c.id?{...x,confirmTargetId:e.target.value,operations:undefined}:x))}><option value="">请选择要更新的记忆</option>{memories.filter(m=>m.type!=='fiction').map(m=><option key={m.id} value={m.id}>{m.text}</option>)}</select></label>}<label>{names[c.type]||'此前留下的内容'}<textarea disabled={disabled} value={c.text} onChange={e=>onCandidates(candidates.map(x=>x.id===c.id?{...x,text:e.target.value,operations:undefined}:x))}/></label>
 ...
 <button className="text-button" disabled={disabled||candidates.some(c=>!c.text.trim()||!c.type||(c.requiresConfirmation&&!c.confirmTargetId))} onClick={()=>{try{const result=onConfirm?.(candidates.map(c=>c.id));if(!result?.okay){setError('未能保存，请保留页面后重试。');return;}setError('');onSaved();}catch{setError('请确认要替换的记忆；内容有变化时请重新核对。');}}}>保存这些内容</button></section>}
