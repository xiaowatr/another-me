...
export default function SupplementaryQuestions({background,onChange,onClose,onGenerate}){
 const [state,setState]=useState(()=>supplementState(background)),[busy,setBusy]=useState(false),[error,setError]=useState(''),[confirmation,setConfirmation]=useState('');
 ...
 async function plan(s){invalidate();const version=revision.current,controller=new AbortController();request.current=controller;const b=store(s);setBusy(true);setError('');
  try{const task=experience.planQuestions(b,controller.signal);pending.current=task;const result=await task;if(version!==revision.current)return;const ids=result.questions.map(q=>q.id);store({...current.current,plan:result,questionMeta:{...current.current.questionMeta,...Object.fromEntries(result.questions.map(q=>[q.id,q]))},shown:[...new Set([...current.current.shown,...ids])],pages:ids.length?[...current.current.pages,ids]:current.current.pages,page:current.current.pages.length});}
