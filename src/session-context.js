...
export function chatClock(background,now=new Date()){const year=now.getFullYear(),birth=Number(background.birthYear);return {mode:'present',asOf:now.toISOString().slice(0,10),year,approxAge:/^\d{4}$/.test(background.birthYear||'')&&birth<=year?year-birth:null};}
export function chatIdentity(background){const t=chatClock(background);return t.approxAge==null?'此刻，走过那条路的我':`现在的我 · 大约${t.approxAge}岁`;}
export function historyRisk(text,background={},memories=[]){
 const factRisk=inspectFactText(text,background,{memories});if(factRisk.length)return factRisk[0];
...
export function safeHistory(history,background,memories=[]){return history.filter(m=>m.role==='user'||!historyRisk(m.content,background,memories)).slice(-12);}
...
export function qualifyCurrentAge(text,background){const age=chatClock(background).approxAge;if(age==null||!/(?:现在|今年|如今|目前)/.test(text))return text;return text.replace(new RegExp('(?<![0-9])'+age+'岁','g'),(match,offset)=>/(?:大约|大概|约|差不多|将近)$/.test(text.slice(Math.max(0,offset-4),offset))||/^左右/.test(text.slice(offset+match.length))?match:'大约'+match);}
