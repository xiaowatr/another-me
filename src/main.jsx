...
function App() {
  ...
  function newLife(){setFormOrigin('lives');setError('');setGenerationFailed(false);setSyncFailed(false);setClearedDraft(null);setTiming(null);setDetailsOpen(false);setSavedNotice('');setEditingProfile(false);setBackground({...empty});setQuestion('');setClarification('');setPage('input');}
  ...
  function cancelGeneration(){if(generation.current){generation.current.abort();generation.current=null;requestLock.current=false;setBusy(false);}}
  ...
  async function next(event, direct=false) {
    event.preventDefault(); if (requestLock.current) return;setGenerationFailed(false);
    ...
    const submitted={...migrateBackground(background),...(hardQuestion(background)?{followupKey:'hard-conflict'}:{}),followupSkipped:direct || (background.followupKey&&!background.followupAnswer?.trim())?'1':background.followupSkipped};
    ...
    try {
      const result = await experience.getStory({ background:submitted, clarification:'',clarificationSkipped:direct || Boolean(submitted.followupKey) || Boolean(submitted.followupSkipped), previousSessionId: null },controller.signal,clientTraceId);
    ...
    } catch (e) { reportStoryTiming(e.timing,controller.signal.aborted?'stopped':'failed');if(generation.current===controller){setError(e.message);setGenerationFailed(true);} }
  ...
  async function send(event,retry=null) {
    ...
    let failure=null;
    try{
      const restored=await experience.restore({background:generatedBackground,story,memories,corrections:explicitCorrections(messages,contextStart),roleRecords:selectRoleRecords(roleRecords(messages,generatedBackground,memories,contextStart),text),history:modelHistory(messages,messageId,contextStart)},sessionId);
  ...
  return <div className={`app-shell ${page==='chat'?'in-chat':''}`}><fieldset className="app-fieldset" disabled={false}>
    ...
    <main><button type="button" className="text-button lives-entry" disabled={busy} onClick={()=>page==='chat'?navigate('life'):returnLives()}>{page==='chat'?'← 故事与背景':'我的那些如果'}</button>{page==='lives'&&<LifeHome library={library} onOpen={switchLife} onNew={newLife} onDelete={deleteLife} onRename={renameLife} notice={savedNotice}/>} {page==='wrap'&&error&&<p role="alert" className="error">{error}</p>}{page==='wrap'&&lifeId&&<WrapUp savedOkay={lifeStorageOkay} life={live.current} working={wrapWorkingId===lifeId} onCandidates={changeCandidates} onSave={saveSelected} onReturn={returnLives} onRetry={endChat}/>} {!lifeStorageOkay && <p className="error" role="alert">浏览器空间不足或不允许存储，当前人生未能保存。请勿关闭页面。</p>}
      ...
      {busy && generation.current && page==='input' && <PostalWaiting onCancel={cancelGeneration}/>}
