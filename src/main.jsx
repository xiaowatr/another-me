...
function App() {
  ...
  const update = (event) => { setClearedDraft(null); const {name,value}=event.target;setStorageOkay(null);setBackground(prev=>({...prev,[name]:value,...(['realityOutcome','hypotheticalDirection','birthYear','forkAge','locationText','choiceReason','details','lifeSituation'].includes(name)?{clarificationAppliedAnswer:'',followupKey:'',followupQuestion:'',followupAnswer:'',followupSkipped:''}:{})})); setQuestion(''); setClarification('');setFollowupOpen(false); };
  ...
  async function next(event, direct=false, recover=null) {
    ...
    try {
      ...
      if (result.kind === 'clarification') { setFollowupOpen(true);setBackground(prev=>({...prev,followupKey:result.hard?'hard-conflict':'model',followupQuestion:publicQuestion(result.question),followupAnswer:'',followupSkipped:''}));return; }
