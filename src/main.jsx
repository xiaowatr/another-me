import {storyParagraphs,storyText} from './story-text.js';
import SupplementaryQuestions from './SupplementaryQuestions.jsx';
...
import {futureScenario} from './input-anchors.js';
import FollowupDialog from './FollowupDialog';
import {initialPage,savePage} from './page-position.js';
import {watchChatViewport} from './chat-viewport.js';
import {pendingStory,resetStoryOperation} from './services/story-task-client.js';
...
function App() {
  ...
  const [storyRecovering,setStoryRecovering]=useState(false);
  ...
  useEffect(()=>{
    ...
    const life={...library.find(l=>l.id===lifeId),conversationContext:lifeContext(generatedBackground,story,memories),chatTime:chatClock(generatedBackground,undefined,story),id:lifeId,name:lifeName,story,background:generatedBackground,messages,draft,memories,candidates,contextStart,eraContext,mode:storyMode,sessionMeta};
  ...
  const liveForm=useRef(background);liveForm.current=background;
  const followup=hardQuestion(background)||(['model','hard-conflict'].includes(background.followupKey)?{question:background.followupQuestion,options:[]}:null);
  function mutateLife(id,transform){const current=live.current?.id===id?live.current:libraryRef.current.find(l=>l.id===id);if(!current)return;const {next,lives:all,okay}=persistLifeChange(draftStorage(),libraryRef.current,live.current?.id,id,current,transform);setLifeStorageOkay(okay);if(!okay){setError('暂时无法保存，请保留当前页面。');return {next:current,okay:false};}libraryRef.current=all;setLibrary(all);if(live.current?.id===id){live.current=next;setMessages(next.messages);setMemories(next.memories);setCandidates(next.candidates);setContextStart(next.contextStart||0);setSessionMeta(next.sessionMeta||emptyReview());}return {next,okay};}
  function changeMemories(next){return mutateLife(lifeId,l=>reviseMemories(l,next));}
  function changeCandidates(next){return mutateLife(lifeId,l=>dismissCandidates(l,next));}
  function returnLives(){setEditingProfile(false);setAboutOpen(false);setPage('lives');}
  async function newLife(){if(generation.current){generation.current.abort();generation.current=null;}if(pendingStory()){try{await experience.cancelStory();}catch(e){setError(e.message);return;}}resetStoryOperation();setFailureId('');setFollowupOpen(false);setShowAgeErrors(false);setStoryRetrying(false);setStoryRecovering(false);setWaitingNote([]);setBusy(false);requestLock.current=false;setBackground(b=>({...b,coreConfirmAttempts:'',clarificationAppliedAnswer:'',followupKey:'',followupQuestion:'',followupAnswer:'',followupSkipped:''}));setFormOrigin('lives');setError('');setGenerationFailed(false);setSyncFailed(false);setClearedDraft(null);setTiming(null);setDetailsOpen(false);setSavedNotice('');setEditingProfile(false);setQuestion('');setClarification('');setPage('input');}
  ...
  async function endChat(retry=false){retry=retry===true;const id=lifeId;if(reviewJobs.current.has(id))return;reviewJobs.current.add(id);experience.memoryDiagnostic({stage:'end_clicked',totalMessages:live.current?.messages.length||0});setWrapWorkingId(id);setAboutOpen(false);const t=activeTurn.current;
    try{
      ...
      if(!job){
        ...
        if(!unchanged){setSavedNotice('记忆已被修改，旧整理结果不能覆盖这些修改。聊天已保留。');return;}
  ...
  const update = (event) => { setClearedDraft(null); const {name,value}=event.target;setStorageOkay(null);setBackground(prev=>({...prev,[name]:value,...(['realityOutcome','hypotheticalDirection','birthYear','forkAge','locationText','choiceReason','details','lifeSituation'].includes(name)?{coreConfirmAttempts:'',clarificationAppliedAnswer:'',followupKey:'',followupQuestion:'',followupAnswer:'',followupSkipped:''}:{})})); setQuestion(''); setClarification('');setFollowupOpen(false); };
  const field = (name, label, placeholder, kind = 'textarea') => <label className="field">{label}{requiredBackgroundFields.includes(name)&&!label.includes('*')?' *':''}{kind === 'input' ? <input required={requiredBackgroundFields.includes(name)} inputMode={['birthYear','forkAge'].includes(name)?'numeric':undefined} aria-invalid={showAgeErrors&&Boolean(ageFieldErrors(background)[name])} name={name} value={background[name]} onChange={update} placeholder={placeholder} maxLength={80}  /> : <textarea required={requiredBackgroundFields.includes(name)} name={name} value={background[name]} onChange={update} placeholder={placeholder} maxLength={1500}  rows={name === 'lifeSituation' ? 2 : name === 'details' ? 4 : 3} />}{showAgeErrors&&ageFieldErrors(background)[name]&&<small role="alert" className="error">{ageFieldErrors(background)[name]}</small>}</label>;
  function confirmCore(event){event.preventDefault();const b=liveForm.current;if(!b.followupAnswer?.trim()||Number(b.coreConfirmAttempts||0)>=2)return;const updated={...b,coreConfirmAttempts:String(Number(b.coreConfirmAttempts||0)+1)};liveForm.current=updated;setBackground(updated);next(event,false,null,updated);}
  async function next(event, direct=false, recover=null, suppliedBackground=null) {
    const background=suppliedBackground||liveForm.current;
    event.preventDefault(); if (requestLock.current) return;setGenerationFailed(false);setStoryRetrying(false);
    const fields = ['realityOutcome','hypotheticalDirection'];setFailureId('');const resolved=applyClarification(background);
    if(!recover&&background.followupKey==='hard-conflict'&&!background.followupAnswer?.trim()&&background.followupQuestion){setFollowupOpen(true);return;}
    if(!recover&&hardQuestion(background)){setBackground(b=>({...b,followupKey:'hard-conflict',followupQuestion:hardQuestion(background).question,followupSkipped:''}));setFollowupOpen(true);setError(background.followupAnswer?.trim()?'还有信息需要确认：'+hardQuestion(background).question:'');return;}
    if(!recover){setShowAgeErrors(true);if(Object.keys(ageFieldErrors(resolved)).length){setError('请核对出生年份和当时年龄。');return;}}
    if(!recover&&formError(resolved)){setError(formError(resolved));return;}
    if(!recover&&coordinateError(resolved)){setError(coordinateError(resolved));return;}
    if(!recover&&fields.some(key=>!background[key]?.trim())){setError('请写下想回到的瞬间，以及这次希望发生的变化。');return;}
    if(!recover&&!editingProfile&&(!direct||supplementState(background).plan?.conflict)){setBackground(resolved);liveForm.current=resolved;setFollowupOpen(false);setSupplementOpen(true);return;}
    setFollowupOpen(false);setError('');setBackground(resolved);
    ...
    const submitted=recover?.input.background||{...migrateBackground(applyClarification(background)),...(hardQuestion(background)?{followupKey:'hard-conflict'}:{}),followupSkipped:!background.followupAnswer?.trim()&&(direct||background.followupKey)?'1':background.followupSkipped};
    setBackground(submitted);setWaitingNote([]);setStoryRecovering(false);
    ...
    try {
      const result = await experience.getStory(recover?.input||{ background:submitted, clarification:'',clarificationSkipped:direct || Boolean(submitted.followupKey) || Boolean(submitted.followupSkipped), previousSessionId: null },controller.signal,clientTraceId,()=>setStoryRetrying(true),note=>{if(generation.current===controller)setWaitingNote(note);},recovering=>{if(generation.current===controller)setStoryRecovering(recovering);});
      ...
      if (result.kind === 'clarification') { setFollowupOpen(true);setBackground(prev=>({...prev,followupKey:'hard-conflict',followupQuestion:publicQuestion(result.question),followupAnswer:'',followupSkipped:''}));return; }
  ...
  return <div className={`app-shell ${page==='chat'?'in-chat':''}`}><fieldset className="app-fieldset" disabled={false}>
    <header className="site-header"><button className="brand" onClick={() => navigate('home')} aria-label="another me 首页"><span className="brand-symbol" aria-hidden="true">✉</span> another me<span className="brand-cn">平行人生邮局</span></button><button type="button" className="text-button header-lives-entry" disabled={busy} onClick={returnLives}>历史记录</button><span className="header-note">另一种人生的来信</span>{status?.mode !== 'real' && <span className="demo-pill">{status?.mode === 'demo' ? '体验原型 / 演示' : '连接检查'}</span>}</header>
    <main>{supplementOpen&&<SupplementaryQuestions background={background} onChange={b=>{liveForm.current=b;setBackground(b);}} onClose={()=>setSupplementOpen(false)} onGenerate={b=>{setSupplementOpen(false);next({preventDefault(){}},true,null,b);}}/>}{page==='chat'&&<button type="button" className="text-button lives-entry" disabled={busy} onClick={()=>navigate('life')}>← 故事与背景</button>}{page==='lives'&&<LifeHome library={library} onOpen={switchLife} onNew={newLife} onDelete={deleteLife} onRename={renameLife} notice={savedNotice}/>} {page==='wrap'&&error&&<p role="alert" className="error">{error}</p>}{page==='wrap'&&lifeId&&<WrapUp savedOkay={lifeStorageOkay} life={live.current} working={wrapWorkingId===lifeId} onCandidates={changeCandidates} onSave={saveSelected} onReturn={returnLives} onRetry={endChat}/>} {!lifeStorageOkay && <p className="error" role="alert">浏览器空间不足或不允许存储，当前人生未能保存。请勿关闭页面。</p>}
      ...
      {page === 'home' && <>
        <div className="hero">
          ...
          <Mailbox onStart={() => {newLife();setFormOrigin('home');}}/>
        </div>
      ...
      {page === 'input' && <section className="compact-form"><div className="form-top"><button type="button" className="text-button" onClick={()=>navigate(formOrigin)}>← 返回</button></div><h1 ref={heading} tabIndex={-1}>回到那个瞬间</h1>{notice}<p className="small">* 为必填。出生年份与当时年龄请填整数；其他选填内容可跳过。</p><form onSubmit={next} noValidate>
        {field('realityOutcome','现实中发生了什么？ *','说说当时的情况，以及后来实际发生的事。还没发生的话，说说现在的情况。')}
        {field('hypotheticalDirection','这次，你想改变哪件事？ *','可以改变一个选择、一种条件，或设想未来。后面的结果可以交给故事展开。')}
        {followupOpen && followup && <FollowupDialog title="先确认这处信息" onClose={()=>setFollowupOpen(false)}><section className="gentle-question" aria-label="一个小问题"><p>{publicQuestion(followup.question)}</p><p className="small">{(hardQuestion(background)||background.followupKey==='hard-conflict')?'请先确认这些互相矛盾的信息，再继续。':'补充后点击下方按钮继续，也可以跳过。'}</p><div className="question-options">{followup.options.map(option=><button type="button" key={option} aria-pressed={background.followupAnswer===option} onClick={()=>setBackground({...background,followupAnswer:option,followupSkipped:''})}>{option}</button>)}</div><label className="sr-only" htmlFor="followup-answer">也可以自己说</label><input id="followup-answer" value={background.followupAnswer || ''} maxLength={500} placeholder="也可以自己说" onChange={e=>setBackground({...background,followupAnswer:e.target.value,followupSkipped:''})}/>{error&&<p role="alert" className="error">{error}</p>}<div className="am-followup-actions">{Number(background.coreConfirmAttempts||0)>=2&&<p role="alert">已确认两轮，请返回修改原文。已填内容会保留。</p>}<button type="button" onClick={confirmCore} className="primary" disabled={busy||!background.followupAnswer?.trim()||Number(background.coreConfirmAttempts||0)>=2}>按这个补充继续生成</button>{!hardQuestion(background)&&background.followupKey!=='hard-conflict'&&<button type="button" className="text-button" disabled={busy} onClick={e=>next(e,true)}>跳过，直接生成</button>}</div></section></FollowupDialog>}
        <section className="optional-fields" aria-label="当时的你"><h2 className="form-group-title">当时的你</h2><div className="coordinate-pair">{field('birthYear','出生年份','例如：2000','input')}<label className="field">性别 *<select name="gender" value={background.gender || ''} onChange={update}><option value="">请选择</option>{['女','男','非二元','不透露'].map(value=><option key={value} value={value}>{value}</option>)}</select></label>{field('forkAge','当时年龄','例如：23','input')}</div><p className="small">选择发生时的年龄；未来设想填预计发生时的年龄。记不清当时年龄，可以填一个大概数字，如23。</p>
        ...
        {field('choiceReason','当时为什么会这样？（选填）','有什么原因或限制？不清楚可以不填。')}
        {field('details','有什么希望故事遵守的？（选填）','例如“不一定和好”“不要改变家人的情况”；没有可以留空。')}
        {field('lifeSituation','补充一点当时的生活（选填）','例如在上学、工作中或暂时休息；不想填也可以。')}
        </section>
