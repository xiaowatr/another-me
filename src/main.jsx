...
import {emptyReview,completeReview,finishReview,dismissCandidates,reviseMemories,sameMemory} from './memory-review';
...
function App() {
  ...
  async function endChat(){const id=lifeId;if(reviewJobs.current.has(id))return;reviewJobs.current.add(id);setWrapWorkingId(id);setPage('wrap');setAboutOpen(false);const t=activeTurn.current;
    try{if(t){await stopReply();await t.finished;await new Promise(resolve=>setTimeout(resolve,0));}if(live.current?.id===id&&live.current.sessionMeta?.review?.end>=live.current.messages.length&&live.current.sessionMeta.review.status==='complete')return;mutateLife(id,l=>({...l,sessionMeta:{...l.sessionMeta,review:{...(l.sessionMeta.review||{}),status:'pending'}}}));
  ...
  useEffect(() => { if (page === 'chat' && messages.length) chatEnd.current?.scrollIntoView({ block: 'nearest' }); }, [messages, page]);
  const navigate = (next) => { if(requestLock.current)return;if(next!=='input')setEditingProfile(false); setError(''); if(next==='input' && page==='life')setBackground(migrateBackground({...empty,...generatedBackground})); setPage(next); };
  ...
  async function send(event,retry=null) {
    event?.preventDefault();const text=(retry?.text ?? draft).trim();if(!text || requestLock.current)return;
  ...
  return <div className="app-shell"><fieldset className="app-fieldset" disabled={busy && !['chat','wrap','lives'].includes(page)}>
    ...
    <main><button type="button" className="text-button lives-entry" disabled={busy} onClick={returnLives}>我的那些如果</button>{page==='lives'&&<LifeHome library={library} onOpen={switchLife} onNew={newLife} onDelete={deleteLife} onRename={renameLife}/>} {page==='wrap'&&error&&<p role="alert" className="error">{error}</p>}{page==='wrap'&&lifeId&&<WrapUp savedOkay={lifeStorageOkay} life={live.current} working={wrapWorkingId===lifeId} onCandidates={changeCandidates} onSave={saveSelected} onReturn={returnLives} onRetry={endChat}/>} {!lifeStorageOkay && <p className="error" role="alert">浏览器空间不足或不允许存储，当前人生未能保存。请勿关闭页面。</p>}
      ...
      {page === 'chat' && story && <section className="chat-page"><div className="chat-title"><div className="avatar">我</div><div><h1 ref={heading} tabIndex={-1}>与另一个自己通信</h1><p>{chatIdentity(generatedBackground)}</p></div><nav className="chat-nav"><button type="button" className="text-button" onClick={()=>navigate('life')}>故事</button><button type="button" className="text-button" onClick={()=>setAboutOpen(true)}>关于我</button></nav></div>{notice}<div className="messages" role="log" aria-label="对话记录" aria-live="polite"><div className="message assistant first-letter"><p>{qualifyCurrentAge(currentOpening(story,generatedBackground),generatedBackground)}</p></div>{messages.map((message, i) => <div className={`message ${message.role}`} key={i}><p>{message.text}</p>{message.contextExcluded && <small>这条旧回复有误，已保留原文。</small>}{message.role==='user' && message.status==='failed' && <button className="text-button" disabled={busy} onClick={e=>send(e,message)}>重试这条消息</button>}</div>)}{chatWaiting && <p role="status">另一个自己正在输入…</p>}<div ref={chatEnd}/></div>
        ...
        }} value={draft} onChange={e => setDraft(e.target.value)} placeholder="写给另一个自己……" rows={2} maxLength={2000}/><button className="primary" disabled={!draft.trim() || busy}>{busy ? '正在回信…' : '发送'} <span>↑</span></button></form>{error && <p role="alert" className="error">{error}</p>}<p className="small">Enter 发送 · Ctrl＋Enter 换行</p></section>}
