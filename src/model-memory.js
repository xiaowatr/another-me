...
export function acceptMemoryResult(life,job,result){
 ...
 const pending=result.operations.filter(o=>o.needsConfirmation).map(o=>({id:o.id,type:o.type||'reality',timeState:o.timeState,text:o.text||'请填写更正后的内容',sourceId:o.sourceId,sourceText:job.input.messages.find(m=>m.id===o.sourceId).text,sourceRole:'user',requiresConfirmation:true,confirmationKind:o.kind,confirmationReason:o.confirmationReason||null,confirmTargetId:o.targetId||'',expectedText:o.expectedText,origin:'model'}));
