...
﻿async function request(path, body, signal, trace) {
  ...
  let response;
  try { response = await fetch(path, { method: body ? 'POST' : 'GET', headers: { 'X-Anonymous-Token':anonymousToken(),...(body?{'Content-Type':'application/json'}:{}),...(trace?{'X-Client-Trace-Id':trace}:{}) }, body: body ? JSON.stringify(body) : undefined, signal: combinedSignal }); }
