import {STORY_MAX_REWRITES,STORY_MAX_MODEL_CALLS,retryableStoryFailure} from '../story-budget.js';
...
export async function storyWithRetry(attempt,{signal,onRetry=()=>{},onFailedAttempt=()=>{}}={}){
 for(let i=0;i<=STORY_MAX_REWRITES;i++){
  try{return await attempt(i);}catch(error){
   if(signal?.aborted||i===STORY_MAX_REWRITES||error.rewriteCount>=STORY_MAX_REWRITES||error.cumulativeCallCount>=STORY_MAX_MODEL_CALLS||!retryableStoryFailure(error))throw error;
