...
export function createStoryClient(request,{pause=wait}={}){return {
 async get(input,signal,trace,onSetting,onRecover=()=>{}){
  ...
  if(!task){const status=await request('/api/status',undefined,signal);const previous=operation(),parent=previous&&(previous.status==='clarification'||previous.signature===signature(input))?previous:null;task={taskId:trace||crypto.randomUUID(),parentTaskId:parent?.taskId||null,continuation:parent?(parent.status==='clarification'?'clarification':'rewrite'):null,instanceId:status.instanceId,input,submitted:false,startedAt:new Date().toISOString()};save(task);created=true;}
  ...
  try{
   ...
   if(state.status==='succeeded'){remember({taskId:task.taskId,signature:signature(input),status:state.result.kind==='clarification'?'clarification':'succeeded',rewriteCount:state.rewriteCount||0});task.result=state.result;if(state.result.kind==='clarification')clearPendingStory(task.taskId);else save(task);return {...state.result,taskId:task.taskId,clientTiming:{clientTraceId:task.taskId,requestId:state.result.requestId||task.taskId,startedAt:task.startedAt,responseReceivedMs:Math.max(0,Date.now()-Date.parse(task.startedAt)),recoveryWaitMs:Math.round(performance.now()-start),recovered:!created}};}
   remember({taskId:task.taskId,signature:signature(input),status:'failed',rewriteCount:state.rewriteCount||0});clearPendingStory(task.taskId);const e=new Error(state.error?.error||'这次生成未完成，填写内容已保留。');Object.assign(e,state.error,{terminal:state.status==='failed',rewriteCount:state.rewriteCount||0,message:state.error?.error||e.message});throw e;
