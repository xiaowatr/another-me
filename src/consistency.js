...
export function explicitCorrections(messages,start=0){const time=messages.filter(m=>m.role==='user'&&m.status!=='pending').map(m=>chatTimeCorrection(m.text||m.content)).filter(Boolean).slice(-1).map(text=>({type:'chat_time',text}));return [...messages.slice(start).filter(m=>m.role==='user'&&!['failed','pending'].includes(m.status)&&!/[？?]/.test(m.text||m.content||'')).flatMap(m=>{const t=m.text||m.content||'';const type=isIdentityCorrection(t)?'identity':/^(?:更正|纠正)(?:一下)?[：，\s]*(?:我的|现实|我)/.test(t)?'reality':/^(?:修改|纠正|更正)(?:一下)?[：，\s]*(?:你的|你|角色|另一个自己)/.test(t)?'fiction':null;return type?[{type,text:t.slice(0,2000)}]:[];}).slice(-19),...time];}
