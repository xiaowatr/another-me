...
export function explicitCorrections(messages,start=0){return messages.slice(start).filter(m=>m.role==='user'&&!['failed','pending'].includes(m.status)&&!/[？?]/.test(m.text||'')).flatMap(m=>{const t=m.text||m.content||'';const type=/^(?:更正|纠正)(?:一下)?[：，\s]*(?:我的|现实|我)/.test(t)?'reality':/^(?:修改|纠正|更正)(?:一下)?[：，\s]*(?:你的|你|角色|另一个自己)/.test(t)?'fiction':null;return type?[{type,text:t.slice(0,2000)}]:[];}).slice(-20);}
