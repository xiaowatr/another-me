...
export const PLANNER_VERSION='questions-v6';
...
export async function planWithGapCheck(background,caller){
 const request=messages=>caller.call('questions',messages,{validateResult:r=>normalizePlan(validateQuestionAssessments(r),background)});
 ...
 if(!plan.conflict&&!s.scenario&&policy.remaining>0&&!policy.currentGroupSkipped){
  ...
  const review=await caller.call('questions',questionReviewMessages(background,plan),{validateResult:r=>validateQuestionReview(r,background)});
  ...
  plan.stop=plan.stop||plan.questions.length===0;response=review;
 }
