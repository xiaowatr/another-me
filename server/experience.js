...
export function createExperience(config, caller, recordLocal = () => {}) {
  ...
  return {
    ...
    restore(input){
      ...
      const restoredCorrections=Array.isArray(data.corrections)?data.corrections.filter(c=>['reality','fiction','identity'].includes(c.type)&&typeof c.text==='string'&&c.text.length<=2000).slice(-20):[];
