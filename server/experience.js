...
export function createExperience(config, caller, recordLocal = () => {}) {
  ...
  return {
    ...
    story: (input,{signal,onSetting,rewriteFeedback}={}) => exclusive('story', async () => {
      ...
      const response = config.mode === 'demo' ? { result: { ...demoStory, kind: 'story', character: demoStory.intro, opening: '【固定演示开场】刚刚关了书店，你想聊些什么？' } } : await caller.call('story', storyMessages(background, input.clarification || '', !input.clarificationSkipped,effective,rewriteFeedback),{signal,diagnosticCaseId,diagnosticRange:diagnosticRange(effective.temporal),diagnosticClock:{asOf:effective.temporal.asOf,startYear:effective.temporal.start?.year??null,startMonth:effective.temporal.start?.month??null},validateResult:result=>{if(result.kind==='clarification' && (background.followupKey || input.clarificationSkipped || input.clarification))throw new AppError('invalid_response',502,{stage:'business',reason:'repeated_clarification'});if(result.kind==='story'){if(openingAddresseeConflict(result.opening,background))throw new AppError('background_conflict',422,{stage:'business',reason:'opening_addressee_conflict',field:'opening'});const conflicts=settingIssues(result,effective);if(conflicts.length)throw new AppError('background_conflict',422,{stage:'business',reason:conflicts[0].reason,field:conflicts[0].field});checkStoryGrounding(result,background,effective.temporal);const review=reviewStory(result,background,[],{strictTime:true});if(review.issues.length)throw new AppError('background_conflict',422,{stage:'business',reason:review.issues[0].reasons[0],field:review.issues[0].field});}}});
