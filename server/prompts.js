...
export function chatMessages(session,message,intent){
 ...
 const history=safeHistory(session.history,session.background,session.memories,session.story).filter(m=>m.role==='user'||(!careerIssues(m.content,evidence).length&&(!beyondStoryEnd(m.content,ctx.chatTime)||/如果|假如|计划|希望|打算/.test(m.content))));
