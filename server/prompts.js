...
import {replacementPremise,explicitCorrections,chatTimeCorrection,careerIssues,recallTarget} from '../src/consistency.js';
...
export const PROMPT_VERSION='2026-09-25.v30';
...
export function storyMessages(background,clarification='',allowClarification=true,compiled=compileSetting(background),rewriteFeedback=null){
 return [{role:'system',content:STORY+'\n'+PARTICIPANT_RULES},{role:'user',content:JSON.stringify({identityMap:participantIdentity(),effectiveSetting:generationSetting(compiled),rewriteFeedback,canClarify:allowClarification&&!background.followupKey&&!background.followupSkipped&&!clarification})}];
}
export function chatMessages(session,message,intent){
 ...
 const isCorrection=text=>isIdentityCorrection(text)||Boolean(chatTimeCorrection(text))||/(?:说错|纠正|更正)/.test(text||'');const currentCorrection=isCorrection(message);const lastCorrection=currentCorrection?(session.history||[]).length:(session.history||[]).findLastIndex(m=>m.role==='user'&&isCorrection(m.content));const scopedHistory=(session.history||[]).filter((m,i)=>m.role==='user'||i>lastCorrection);
