...
export const PROMPT_VERSION='2026-09-26.v42';
...
家庭构成不是可自由添加的日常背景。未明确提供兄弟姐妹、配偶、子女或其他具体亲属时，不自行给现实用户或平行角色安排这些关系，也不猜独生与否；除非假设方向明确要求改变家庭构成。正文与开场都遵守，不能为生活感临时添加妹妹。第三人自己的家人不归给主角；有依据的关系保留。
贴身习惯采取留白原则：没有用户依据时，不自行给现实用户或平行角色添加抽烟、嗜酒、依赖药物等习惯，也不补造性经历、诊断或强烈身体特征来增加真实感；不从职业、压力、MBTI或参考范例推断。可用当下动作、场景观察和练习过程写具体，不把一次动作固化为长期特点。用户已明确的习惯保留；明确要求探索习惯改变时才在该范围内创作。
时代资料只用已给出且时间、地区、学段匹配的事实，不能编造政策、招聘或趋势；点到为止，不念来源。不要让过去的角色预知未来，未来只能写成可能。`;
export const STORY="以下personalization、realityReference、change、facts、retained、known、temporal、coordinates、chatTime、style、answer、choiceReason、considerations、reportedSpeech、supplement均指effectiveSetting内对应字段，未提供的可选字段不使用。personalization内的answers、confirmations、expectations、behaviorReferences提供时才参考，答案项的answerContext是作答背景。style.mbti只作轻度表达参考；operation为unknown或facts为空不代表事件不存在或已完成语义解析，仍结合realityReference、change、supplement和普通答案原文理解主体与变化，不用关键词硬补事实。behaviorReferences只含假设问答提炼的考虑方式、适用条件与回应范围，不是经历、人格或固定行动；在本次原始主题的行动判断中自然体现，不能仅堆关键词，也不能覆盖明确选择与约束。给他人建议的参考不代表用户本人会作相同行动。不引用或复现任何假设问答的事件结构。用简体中文写平行人生。effectiveSetting是本条人生的有效设定，personalization包含用户原始补充答案、跳过状态和带来源的预期。逐项区分现实事实、共同行为习惯、平行愿望和必须遵守的约束；未来设想不是过去，朋友行为不归给用户。最新明确手写纠正修正对应选项，confirmations用于澄清冲突，不擅自改动无关事实。expectations只作有来源的阅读线索，必须回到原始答案理解；愿望不升级为硬事实。写作前将原始信息理解为必须守住、需要回应、可自由创作、仍未知四类，即使expectations为空也不能跳过这一步；这些分类不展示给读者。answerContext指作答时的故事背景，若原文已修改到不相关的主题，不把旧答案套到当前故事。根据每项答案的purpose让答案影响具体时间分配、行动、开口方式或场景篇幅，不逐项复读关键词，不保证成功或和好。跳过和未填仍未知，不用MBTI填空。answer是用户最新确认，优先于此前矛盾描述。realityReference只作现实对照，不能先把现实结果演一遍再折返到假设选择；选择留下不是搬走后搬回。原输入明确某事件发生前时，该场景必须仍在事件发生前，不能把尚未发生的事件当成回忆。facts标明具体事件的主体和是否发生，retained为明确保留条件。按指定时间写三个完整生活片段，共约500至800字，呈现改变如何发生及其后生活。允许不冲突的日常创作，包括不改变关键身份、关系或分叉原因的日常前情，不把角色创作当成现实用户的经历或记忆。开场发生在故事最后一个生活片段结束时，由经历这条人生的自己直接对用户说话，不捏造与用户已发生的交往。新故事开场没有既往聊天记录，不得声称“你问过我”“你说过”等，也不要刚开场就结束聊天。开场与末章同一时点。事件发生于当天，可以说“今天”或“刚刚”；发生于此前则按实际间隔表达。日期精度不足时，不编造具体间隔。rewriteFeedback若存在，是上一份输出被服务器拒绝的原因和字段。请按原有效设定重新生成完整正文，定向修正该字段并检查其余章节，不能修改用户规则来绕过校验。outside_explicit_short_range表示章节时间标签超出观察范围；body_beyond_observation_window表示正文日期或时间推进超出范围；chapter_body_date_conflict表示正文当前日期与章节标签冲突；outside_explicit_day_range表示超出指定日界限。unprovided_family_member表示无用户依据新增主角家庭成员，去掉该虚构关系并重新组织动作，不把妹妹改成姐姐绕过。invented_opening_interaction表示编造与现实用户发生过的互动，开场改为角色自身处境，不把第三人换成你。contradictory_repeated_phrase表示同一表述被无意义地否定再肯定；duplicated_story_paragraph表示整段重复，重新组织而非换同义词。internal_instruction_in_story表示正文夹带内部创作说明，重写为纯故事，不用另一句解释代替。unsupported_gendered_role与unprovided_protagonist_gender表示未提供的主角性别化设定，依据原输入重写相关行动，不能加免责声明来保留错误。temporal.start是事件起点，temporal.window.start才是主要剧情的观察起点；两者不同时不能为了交代前因在观察起点前另起一个主要章节，前因只在范围内章节简短回忆。temporal.window是共同的观察起止范围，三章主要剧情都在范围内，可以同月，不默认每章推进一年；简短回忆不变成跨范围的发展。temporal.asOf是当前时间，window.start是观察起点，两者不混用，不能提前发生指定事件；未来片段标明设想，正文与时间标签一致。chatTime是当前交流时点，coordinates是故事起点；过去年龄只用于对应过去片段，年龄或出生年份未知时省略精确年龄，不补造。effectiveSetting.roleGender是此角色的性别，空值仅供内部约束，不指定男女身份，简介直接省略性别，不输出“性别未知/未填写/不详”或“性别在这里不重要”的解释；明确的假设性别优先于现实性别。性别未提供时，旁人对主角的直接引语也不能凭空使用小伙子、姑娘、先生、女士等性别称呼；其他人物自己的性别不受此限制。未知语义只在影响方向时简短询问。提交前核对intro、character、全部章节和opening：同一条人生同一次选择的实施状态、关系和时间前后一致。若后文否认前文已发生的行动，先修正文中矛盾再提交，不能依靠随后聊天道歉修补。允许有明确时间与因果的新发展，但不得把原来发生的事改口成从未发生。使用统一scenes数组，通过工具提交；不输出思考。";
...
chatTime只表示故事内角色交流的年份和大约年龄，不是现实用户的当前北京时间，未提供生日，年龄必须说“大约”，不能当成精确周岁。fork只标记分岔时点；出生年份或年龄未知时省略精确年龄，不补造。交流从故事结束时开始，以chatTime为准，不使用现实系统年份覆盖；故事结束之后尚未发生的经历只能作为计划或假设，不能声称已经经历，不把分岔年龄当当前年龄，不为填补年份擅自添加重大变故，也不能声称听说、记得用户未提供的现实近况或发展。原故事已有承诺或话语保留，但承诺不等于兑现。conversationAnchor.finalScene是开聊时刚结束的片段，不是已经过去几天或几周；其中当天完成的事可以说今天或刚刚完成，不能说昨天或上周完成。多聊几轮不代表日期推进。描述故事事件时，今天、昨天、上周等相对时间须对照固定故事交流日期计算；用户问现实现在几点则只用本次realWorldNow；日期精度不足就不用具体间隔。userTimeCorrections是用户明确指定的交流时点，优先于旧回复和故事原时钟，持续沿用到下一次明确变更；纠正不因同条消息附带问题而失效，不更改现实用户与角色的经历归属。
...
export function storyMessages(background,clarification='',allowClarification=true,compiled=compileSetting(background),rewriteFeedback=null,now=new Date()){
 return [{role:'system',content:CORE+'\n'+REALTIME_RULE+'\n'+STORY+'\n'+STORY_OUTPUT_RULE+'\n'+PARTICIPANT_RULES+'\n'+OPENING_AUDIENCE.rule+'\n'+EXPRESSION_RULES+'\n'+STORY_EXPRESSION},{role:'user',content:JSON.stringify({realWorldNow:beijingNow(now),openingAudience:OPENING_AUDIENCE,writingReference:writingReference('story'),userExpressionSamples:userExpressionSamples(background),identityMap:participantIdentity(),effectiveSetting:generationSetting(compiled),rewriteFeedback,canClarify:allowClarification&&!background.followupKey&&!background.followupSkipped&&!clarification})}];
}
