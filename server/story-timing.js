...
export function makeStoryTrace(res,clientId,{directory='.local',emit=line=>console.info(line),env=process.env,detached=false,onModelStart=()=>{}}={}){
 ...
 trace.complete=()=>finish('task_finished');
 if(!detached){res.once('finish',()=>finish('response_finished'));res.once('close',()=>{if(!res.writableFinished)finish('connection_closed');});}
