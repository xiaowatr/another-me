...
## 历史入口命名
顶部入口和列表标题改为历史记录；前端index-BOCgDlJM.js，后端版本不变。构建及本地资源检查通过，本轮未部署。

## v91 补充题有限恢复
选题/独立审查各最多3次，成功阶段复用，正常2次、极限6次模型调用；暂时错误重试，余额/配置/内容拒绝不循环。HTTP网络恢复接回同一任务，失败终态不刷新预算；耗尽后沿用保留原答继续故事。明确审查证据引用来源，仅兼容可核对的引号/空白。37项定向及两条隔离页面路径通过，真实模型/真机未验证。前后端一起更新：v91/questions-v8、index-CDWq2P0j.js，Prompt v44保持；本轮未部署。

## v92 回答与开场连续性
强化频率/投入约束、末章最终状态与最新消息承接，有限拦截虚构既往提问。14项定向及构建通过，真实生成/真机未验证；可选独立故事审查仍未开启，不增加调用。前后端一起更新v92/Prompt v45/style-reference-v5、index-l8ezEGhi.js；questions-v8保持，本轮未部署。
